<?php
	if($_POST['submit']){
		...
		$database->query('UPDATE tasks SET task_name=:task_name,task_body=:task_body,due_date=:due_date,list_id=:list_id,is_complete=:is_complete');
		...
		if($database->rowCount()){
			echo '<p>Your Task has been updated</p>';
		}
	}
?>
<?php
...
<form action = "<?php $_SERVER['PHP_SELF']; ?>" method="post">
	...
	<textarea rows="5" cols ="50" name="task_body"><?php echo $row['task_body']; ?></textarea>
	...
	<select name="list_id">
		...
		<?php foreach ($rows as $list): ?>
			<option value="<?php echo $list['id']?>" <?php if($list['id']==$task_id){echo 'selected';} ?>></option>
		<?php endforeach;?>
	</select>
	<br/>
